...
if [[ -f "${OUTDIR}/${SAMPLE}.featuremap.vcf.gz.csi" && "${OUTDIR}/${SAMPLE}.featuremap.vcf.gz" -ot "${OUTDIR}/${SAMPLE}.featuremap.vcf.gz.csi" ]]; then
    echo "Index found for ${OUTDIR}/${SAMPLE}.featuremap.vcf.gz - proceeding with extracting exome..."
else 
    echo "No index found for ${OUTDIR}/${SAMPLE}.featuremap.vcf.gz or index is older than VCF- generating index with bcftools..."
    bcftools index --threads ${SLURM_CPUS_PER_TASK} ${OUTDIR}/${SAMPLE}.featuremap.vcf.gz
fi

SOURCE=/data/Kastner_PFS/scripts/pipelines/ppmseq
# ENV=${SOURCE}/ppmseq-env
# if [ -d "$ENV" ]; then
#     echo "Conda environment found at $ENV - proceeding with filtering..."
# else
#     echo "Error: Conda environment not found at $ENV - please create the environment and install the required packages before running this script."
#     exit 1
# fi  
...
bcftools view --threads ${SLURM_CPUS_PER_TASK} \
    -R ${REGIONS} \
    -Oz -o ${OUTDIR}/${SAMPLE}.featuremap.exome.vcf.gz \
...
AMBIG_REF=$(bcftools query -f '%CHROM,%POS,%REF' ${OUTDIR}/${SAMPLE}.featuremap.exome.vcf.gz | grep -e B -e K -e M -e N -e R -e S -e W -e Y -w) 
...
vcf2parquet \
    --input ${OUTDIR}/${SAMPLE}.featuremap.exome.vcf.gz \
    convert \
    --output ${OUTDIR}/${SAMPLE}.featuremap.exome.parquet
...
UG_SAMPLE=$(bcftools query -l ${OUTDIR}/${SAMPLE}.featuremap.exome.vcf.gz | head -n 1)
echo "Ultima Genomics sample name extracted from VCF: ${UG_SAMPLE}"

python ${SOURCE}/SNVQ_filter.py \
    -s ${SAMPLE} \
    -u "${UG_SAMPLE}"

# Converting the parquets from SNVQ_filter.py to VCFs 

python ${SOURCE}/parquet_to_vcf.py \
    -s ${SAMPLE} \
    -u "${UG_SAMPLE}" \
    -p ${OUTDIR}/${SAMPLE}.featuremap.exome.SNVQ55_stMIXED.parquet

python ${SOURCE}/parquet_to_vcf.py \
    -s ${SAMPLE} \
    -u "${UG_SAMPLE}" \
    -p ${OUTDIR}/${SAMPLE}.featuremap.exome.SNVQ60.parquet
